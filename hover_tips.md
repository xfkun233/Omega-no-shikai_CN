...
## 用法
- 方式一：使用宏（默认字号 20，术语以淡橙色 `0xFFCC66` 显示）
```
[tip note="说明文本"]术语[endtip]
```
- 方式二：直接链接调用（可自定义字号）
```
...
- 修改（方式一）：
```
若词源核心主题本就基于[tip note="通过模仿目标行为或形态来达成效果的巫术"]模仿巫术[endtip]的脉络，[r]
```
...
@endscript
[tip note=&tf.note]如金箔猫[endtip]
```

## 批量应用建议
- 将括号内行内译注摘出作为 `说明文本`，用 `[tip]...[endtip]` 包裹术语。
...
- 宏支持 `color`，默认淡橙色 `0xFFCC66`；可自定义，如 `0xff0000`。
- 文本中的 `[r]` 会被解析为换行。
